...
def API_SVD(x_inl, y_inl, x_ANY, y_ANY):
    ...
    P_SVD=0.07 
    n_blocks = len(y_inl) 
    block_size = int(len(y_inl) / n_blocks)  
    circulant_blocks = []
    for i in range(n_blocks):
        circulant_blocks.append(y_inl[i*block_size:(i+1)*block_size])
        
    inlet_circulant = circulant(circulant_blocks)
    V, S, U = svd(np.matrix.transpose(inlet_circulant))
    S = np.diag(S)
    truncate=P_SVD*np.max(S)
    S[S<truncate]=0
    S[S>truncate]=1/(S[S>truncate])
...
for x in range(0,1):  
    ...
    import re
    import glob
    proj = np.empty((512,384,1762), dtype=np.float32) 
    ...
    for file, idx in enumerate(filelist):
       proj[:,:,file] = np.fromfile(open(filelist[file], 'rb'), dtype=np.float32).reshape((512,384)) 
    plt.imshow(proj[:,:,900])
    ...
    time_vector=np.arange(0,1.762,0.001) 
    ...
    I_inlet_results=np.array(API_parameters(time_vector, TDC_inlet_average)) 
    I_inlet_results[0]=1
    ...
    residual_function1_ =residual_function_/np.max(residual_function_) 
    k=signal.fftconvolve(TDC_inlet_average, residual_function_) 
    ...
    plt.plot(time_vector, residual_function1_, color='red', label='Plot 1') 
    plt.xlabel(('Time'),  fontsize="15")
